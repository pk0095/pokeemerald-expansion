...
void GenerateMonFromTrainerMon(struct Pokemon *mon, const struct TrainerMon *trainerMon, struct TrainerGenerator *trainer)
{
    ...
    u16 scaledSpecies = trainerMon->species;
    u8 scaledLevel = trainerMon->lvl;
    ...
    if (scalingConfig != NULL)
    {
        ...
        scaledSpecies = ValidateSpeciesForLevel(trainerMon->species, scaledLevel, scalingConfig->manageEvolutions);
    }

    CreateMon(mon, scaledSpecies, scaledLevel, personality, trainer->otID);
    ...
    {
        TryApplyScaledTrainerEVs(mon, scalingConfig, scaledLevel);
    }

    SetMonData(mon, MON_DATA_IVS, &trainerMon->iv);
    CustomTrainerPartyAssignMoves(mon, trainerMon);
    MaybeFilterTrainerMoves(mon, scaledSpecies, scaledLevel, scalingConfig);
